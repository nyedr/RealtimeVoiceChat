...
def setup_logging(level: int = logging.INFO) -> None:
    ...
    Replaces any existing handlers with our custom formatted handler to ensure
    consistent formatting across all log messages. Applies a `CustomTimeFormatter`
    to display timestamps as MM:SS.cs and uses ANSI colors for different log
    message parts (timestamp, logger name, level, message).
    ...
    # Remove any existing handlers to ensure our custom formatter is used
