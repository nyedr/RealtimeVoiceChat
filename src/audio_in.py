...
class AudioInputProcessor:
    ...
    def __init__(
        ...
        self.recording_start_callback: Optional[Callable[[
    ...
    async def _run_transcription_loop(self) -> None:
        ...
        while True:  # Loop restored to continuously call transcribe_loop
            try:
                ...
                logger.debug(
                    "👂✅ TranscriptionProcessor.transcribe_loop completed one cycle.")
                ...
                logger.error(
                    f"👂💥 Transcription loop ({task_name}) encountered a fatal error: {e}. Loop terminated.", exc_info=True)
                self._transcription_failed = True  # Set failure flag
                break  # Exit the while loop, stopping retries

        logger.info(
            f"👂⏹️ Background transcription task ({task_name}) finished.")
        ...
        while True:
            try:
                ...
                if self._transcription_failed:
                    logger.error(
                        "👂🛑 Transcription task failed previously. Stopping audio processing.")
                ...
                if self.transcription_task and self.transcription_task.done() and not self._transcription_failed:
                    ...
                    if task_exception and not isinstance(task_exception, asyncio.CancelledError):
                        ...
                        logger.error(
                            f"👂🛑 Transcription task finished with unexpected error: {task_exception}. Stopping audio processing.", exc_info=task_exception)
                ...
                if audio_data is None:
                    logger.info(
                        "👂🔌 Received termination signal for audio processing.")
                ...
                logger.error(
                    f"👂💥 Audio processing error in queue loop: {e}", exc_info=True)
    ...
    def shutdown(self) -> None:
        ...
        if self.transcription_task and not self.transcription_task.done():
            ...
            logger.info(
                "👂✅ Transcription task already done or not running during shutdown.")
