...
class TranscriptionProcessor:
    ...
    def __init__(
            ...
            realtime_transcription_callback: Optional[Callable[[
                str], None]] = None,
            full_transcription_callback: Optional[Callable[[
                str], None]] = None,
            potential_full_transcription_callback: Optional[Callable[[
                str], None]] = None,
            potential_full_transcription_abort_callback: Optional[Callable[[
            ], None]] = None,
            potential_sentence_end: Optional[Callable[[str], None]] = None,
            before_final_sentence: Optional[Callable[[
                Optional[np.ndarray], Optional[str]], bool]] = None,
            ...
            local: bool = True,
            # Note: This seems unused in the original code provided
            tts_allowed_event: Optional[threading.Event] = None,
            pipeline_latency: float = 0.5,
            # Allow passing custom config
        ...
        self.recorder: Optional[AudioToTextRecorder |
                                AudioToTextRecorderClient] = None
        ...
        if USE_TURN_DETECTION:
            logger.info(
                f"👂🔄 {Colors.YELLOW}Turn detection enabled{Colors.RESET}")
        ...
        def monitor():
            ...
            while not self.shutdown_performed:
                ...
                if self.recorder and speech_end_silence_start is not None and speech_end_silence_start != 0:
                    ...
                    latest_pipe_start_time = silence_waiting_time - \
                        self.pipeline_latency - self._PIPELINE_RESERVE_TIME_MS
                    ...
                    if hot_condition_met and not hot:
                        ...
                        if self._is_recorder_recording():  # Check if still recording before aborting
                            print(
                                f"{Colors.CYAN}COLD (during silence){Colors.RESET}")
                ...
                # Short sleep to prevent busy-waiting dominating CPU
                time.sleep(0.001)
    ...
    def on_new_waiting_time(
        ...
        """
        if self.recorder:
            current_duration = self._get_recorder_param(
            ...
            if current_duration != waiting_time:
                ...
                self._set_recorder_param(
                    "post_speech_silence_duration", waiting_time)
        else:
            logger.warning(
                "👂⚠️ Recorder not initialized, cannot set new waiting time.")
    ...
    def transcribe_loop(self) -> None:
        ...
        def on_final(text: Optional[str]):
            if text is None or text == "":
                logger.warning(
                    "👂❓ Final transcription received None or empty string.")
                return

            self.final_transcription = text
            logger.info(
                f"👂✅ {Colors.apply('Final user text: ').green} {Colors.apply(text).yellow}")
        ...
        if self.recorder:
            ...
            if hasattr(self.recorder, 'text'):
                # type: ignore # Assume method exists
            ...
            logger.error(
                "👂❌ Cannot set final callback: Recorder not initialized.")
    ...
    def abort_generation(self) -> None:
        ...
        self.potential_sentences_yielded.clear()
        logger.info(
            "👂⏹️ Potential sentence yield cache cleared (generation aborted).")
    ...
    def perform_final(self, audio_bytes: Optional[bytes] = None) -> None:
        ...
        if self.recorder:  # Check if recorder exists, primarily as a gatekeeper
            ...
            logger.info(
                f"👂❗ {Colors.apply('Forced Final user text: ').green} {Colors.apply(current_text).yellow}")
            self.sentence_end_cache.clear()
            self.potential_sentences_yielded.clear()
            ...
            logger.warning(
                "👂⚠️ Cannot perform final: Recorder not initialized.")
    ...
    def _normalize_text(self, text: str) -> str:
        ...
        # Keep spaces for SequenceMatcher
    ...
    def detect_potential_sentence_end(self, text: Optional[str], force_yield: bool = False, force_ellipses: bool = False) -> None:
        ...
        ends_with_punctuation = any(stripped_text_raw.endswith(p)
        ...
        if should_yield:
            ...
            if not already_yielded:
                ...
                logger.info(
                    f"👂➡️ Yielding potential sentence end: {stripped_text_raw}")
    ...
    def get_audio_copy(self) -> Optional[np.ndarray]:
        ...
        if not self.recorder:
            logger.warning(
                "👂⚠️ Cannot get audio copy: Recorder not initialized.")
        ...
        try:
            ...
            full_audio_array = np.frombuffer(
                b''.join(frames_data), dtype=np.int16)
            ...
            full_audio = full_audio_array.astype(
    ...
    def _create_recorder(self) -> None:
        ...
        def stop_recording() -> bool:
            ...
            if self.before_final_sentence:
                ...
                try:
                    ...
                    result = self.before_final_sentence(
                        audio_copy, self.realtime_text)
                    return result if isinstance(result, bool) else False
                except Exception as e:
                    logger.error(
                        f"👂💥 Error in before_final_sentence callback: {e}", exc_info=True)
        ...
        def on_partial(text: Optional[str]):
            ...
            if stripped_partial_user_text_new != self.stripped_partial_user_text:
                ...
                logger.info(
                    f"👂📝 Partial transcription: {Colors.CYAN}{text}{Colors.RESET}")
            ...
            # Log less critical updates differently (optional, uncomment if needed)
            else:
                logger.debug(
                    f"👂📝 Partial transcription (no change after strip): {Colors.GRAY}{text}{Colors.RESET}")
        ...
        # Use print for formatted JSON as logger might mangle it
    ...
    def feed_audio(self, chunk: bytes, audio_meta_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.recorder and not self.shutdown_performed:
            try:
                ...
                logger.debug(
                    f"👂🔊 Fed audio chunk of size {len(chunk)} bytes to recorder.")
            except Exception as e:
        ...
        if not self.shutdown_performed:
            ...
            if self.recorder:
                ...
                try:
                    ...
                    logger.error(
                        f"👂💥 Error during recorder shutdown: {e}", exc_info=True)
                finally:
                    self.recorder = None
            else:
            ...
            if USE_TURN_DETECTION and hasattr(self, 'turn_detection') and hasattr(self.turn_detection, 'shutdown'):
                ...
                try:
                    # Example: Assuming TurnDetection has a shutdown method
