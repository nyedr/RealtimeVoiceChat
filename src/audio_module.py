...
ENGINE_SILENCES = {
    ...
    "kyutai":  Silence(comma=0.3, sentence=0.6, default=0.3),
}
...
class AudioProcessor:
    ...
    def __init__(
        ...
        self.silence = ENGINE_SILENCES.get(
            engine, ENGINE_SILENCES[self.engine_name])
        ...
        if engine == "coqui":
            ...
            self.engine = KyutaiEngine(
                device="cuda" if __import__(
                    "torch").cuda.is_available() else "cpu",
                temp=0.6,
                cfg_coef=2.0,
                debug=False
            )
            # Use default voice - can be configured later if needed
        else:
        ...
        if self.engine_name == "coqui" and hasattr(self.engine, 'set_stream_chunk_size') and self.current_stream_chunk_size != QUICK_ANSWER_STREAM_CHUNK_SIZE:
            logger.info(
                f"👄⚙️ Setting Coqui stream chunk size to {QUICK_ANSWER_STREAM_CHUNK_SIZE} for initial setup.")
        ...
        def on_audio_chunk_ttfa(chunk: bytes):
            ...
            if ttfa is None:
                ...
                logger.debug(
                    f"👄⏱️ TTFA measurement first chunk arrived, TTFA: {ttfa:.2f}s.")

        self.stream.feed(
            "This is a test sentence to measure the time to first audio chunk.")
        ...
        if ttfa is not None:
            ...
            logger.warning(
                "👄⚠️ TTFA measurement failed (no audio chunk received).")
    ...
    def synthesize(
        ...
        def on_audio_chunk(chunk: bytes):
            ...
            if on_audio_chunk.first_call and self.engine_name == "orpheus":
                ...
                try:
                    ...
                    logger.warning(
                        f"👄⚠️ {generation_string} Quick Error analyzing audio chunk for silence: {e}")
            ...
            if put_occurred_this_call and not on_audio_chunk.callback_fired:
                if self.on_first_audio_chunk_synthesize:
                    try:
                        logger.info(
                            f"👄🚀 {generation_string} Quick Firing on_first_audio_chunk_synthesize.")
                        self.on_first_audio_chunk_synthesize()
                    except Exception as e:
                        logger.error(
                            f"👄💥 {generation_string} Quick Error in on_first_audio_chunk_synthesize callback: {e}", exc_info=True)
        ...
        logger.info(
            f"👄▶️ {generation_string} Quick Starting synthesis. Text: {text[:50]}...")
        ...
        while self.stream.is_playing() or not self.finished_event.is_set():
            if stop_event.is_set():
                ...
                # Wait for stream stop confirmation
    ...
    def synthesize_generator(
       ...
       `on_first_audio_chunk_synthesize` callback when the first valid audio chunk is queued.
        ...
        start = time.time()
        # Separate timer for generator synthesis
        ...
        def on_audio_chunk(chunk: bytes):
            ...
            if stop_event.is_set():
                logger.info(
                    f"👄🛑 {generation_string} Final audio stream interrupted by stop_event.")
                return
            ...
            if on_audio_chunk.first_call and self.engine_name == "orpheus":
                ...
                try:
                    ...
                    logger.warning(
                        f"👄⚠️ {generation_string} Final Error analyzing audio chunk for silence: {e}")
            ...
            if on_audio_chunk.first_call:
                ...
                ttfa_actual = now-start
                logger.info(
                    f"👄🚀 {generation_string} Final audio start. TTFA: {ttfa_actual:.2f}s.")
            else:
            ...
            if put_occurred_this_call and not on_audio_chunk.callback_fired:
                if self.on_first_audio_chunk_synthesize:
                    try:
                        logger.info(
                            f"👄🚀 {generation_string} Final Firing on_first_audio_chunk_synthesize.")
                        self.on_first_audio_chunk_synthesize()
                    except Exception as e:
                        logger.error(
                            f"👄💥 {generation_string} Final Error in on_first_audio_chunk_synthesize callback: {e}", exc_info=True)
                on_audio_chunk.callback_fired = True
        ...
        logger.info(
            f"👄▶️ {generation_string} Final Starting synthesis from generator.")
        self.stream.play_async(**play_kwargs)
        ...
        while self.stream.is_playing() or not self.finished_event.is_set():
            if stop_event.is_set():
                self.stream.stop()
                logger.info(
                    f"👄🛑 {generation_string} Final answer synthesis aborted by stop_event.")
                buffer.clear()
                # Wait for stream stop confirmation
                self.finished_event.wait(timeout=1.0)
                return False  # Indicate interruption
